...
fn generator(input: &str) -> HashMap<u16, Bot> {
    let bots =
        input
            .lines()
    ...
    input
        ...
        .fold(bots, |mut bots, l| {
            let parts = l.split(' ').collect_vec();
            bots.get_mut(&parts[5].parse().unwrap())
                .unwrap()
                .add_chip(parts[1].parse().unwrap());
            bots
        })
}

fn part_1(bots: HashMap<u16, Bot>) -> u16 {
    let mut bots = bots.clone();
    let mut outputs = HashMap::new();
    loop {
        if let Some(id) = bots.iter().find_map(|(id, bot)| {
...
fn part_2(bots: HashMap<u16, Bot>) -> u32 {
    let mut bots = bots.clone();
    let mut outputs = HashMap::new();
    loop {
...
fn apply(bots: &mut HashMap<u16, Bot>, outputs: &mut HashMap<u16, u16>) {
    let has_double = bots
        ...
        .filter_map(|(&id, bot)| {
            bot.chips
                .0
                .and_then(|c1| bot.chips.1.map(|c2| (id, c1, c2)))
        })
        .collect_vec();
