...
fn generator(input: &str) -> Vec<HashMap<String, String>> {
    input
        .split("\n\n")
        .map(|s| {
            s.split_whitespace()
...
fn part_1(input: Vec<HashMap<String, String>>) -> usize {
    input.iter().filter(|p| has_fields(p)).count()
}

fn part_2(input: Vec<HashMap<String, String>>) -> usize {
    input
        .iter()
        .filter(|p| has_fields(p) && p.iter().all(|(k, v)| is_field_valid(k, v)))
        .count()
}
